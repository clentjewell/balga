...
for(const [slug,url] of [['services','/services/'],['projects','/projects/']]){
  const p=await ctx.newPage();
  await p.goto('http://127.0.0.1:4321'+url,{waitUntil:'load'});
  ...
  await p.screenshot({path:`qa/rebuild/${slug}-desktop.png`,fullPage:true}); console.log('shot',slug); await p.close();
}
await b.close();
