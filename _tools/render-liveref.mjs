...
const slugs = process.argv.slice(2).length ? process.argv.slice(2) : ['home'];
...
for (const slug of slugs){
  ...
  await p.goto(`http://127.0.0.1:${PORT}/page/${slug}`,{waitUntil:'load',timeout:30000}).catch(()=>{});
  await p.addStyleTag({content:`*{animation:none !important; transition:none !important;} .elementor-invisible{opacity:1 !important; transform:none !important;}`});
  await p.evaluate(()=>{
    document.querySelectorAll('[data-settings]').forEach(el=>{ try{ const s=JSON.parse(el.getAttribute('data-settings')); if(s.background_background==='slideshow' && s.background_slideshow_gallery?.length){ const url=s.background_slideshow_gallery[0].url.replace('https://balgadesigns.com.au',''); el.style.backgroundImage=`url('${url}')`; el.style.backgroundSize='cover'; el.style.backgroundPosition='center'; } }catch(e){} });
  ...
  await p.screenshot({path:`qa/reference/${slug}-live.png`,fullPage:true});
  console.log('live ref', slug, 'captured');
  await p.close();
}
